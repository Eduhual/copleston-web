...
foreach ($file in $files) {
    ...
    if (-not $content.Contains("firebase-auth-compat")) {
        $content = $content.Replace("</head>", "    <!-- Firebase Auth Compat -->`n    <script src=`"https://www.gstatic.com/firebasejs/10.11.0/firebase-app-compat.js`"></script>`n    <script src=`"https://www.gstatic.com/firebasejs/10.11.0/firebase-auth-compat.js`"></script>`n</head>")
    }
    
    $content = $content -replace '(?s)<nav>\s*<ul>.*?</ul>\s*</nav>', "<nav>`n            <ul>`n                <li><a href=`"./index.html#vanguardia`">Inicio</a></li>`n                <li><a href=`"./manifiesto.html`">MANIFIESTO</a></li>`n                <li><a href=`"./nosotros.html`">NOSOTROS</a></li>`n                <li><a href=`"#`" class=`"open-modal-link`" data-target=`"article-modal`">Artículos</a></li>`n                <!-- <li><a href=`"./eventos.html`">Eventos</a></li> -->`n                <li><a href=`"./contacto.html`">Contacto</a></li>`n                <li><a href=`"#`" id=`"auth-btn`" class=`"btn-auth-solid`">Acceder</a></li>`n            </ul>`n        </nav>"
    
    $content = $content -replace '(?s)<!-- Modal de Autenticación Premium -->.*?</div>\s*</div>', ''
    $content = $content -replace '(?s)<!-- Modal de Mi Perfil Premium -->.*?</div>\s*</div>', ''
    
    $content = $content -replace '(?s)<!-- Modales Flotantes -->\s*.*?\s*<div id="article-modal"', "<!-- Modales Flotantes -->`n    <div id=`"article-modal`""
    ...
    <div id="profile-modal" class="modal-overlay">
        <div class="modal-content light-glass" style="max-width: 380px; padding: 3rem 2rem; border-radius: 20px;">
            ...
            <div class="profile-header text-center" style="display: flex; flex-direction: column; align-items: center; gap: 1rem;">
                ...
                <!-- Botón de Cerrar Sesión (Minimalista Rojo) -->
...
"@
    ...
    $content = $content.Replace("<!-- Modales Flotantes -->", $modals)
